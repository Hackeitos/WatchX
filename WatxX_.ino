...
//#define nSec(_time_) (_time_ % SECS_PER_MIN)
//#define nMin(_time_) ((_time_ / SECS_PER_MIN) % SECS_PER_MIN)
//#define nHour(_time_) (( _time_% SECS_PER_DAY) / SECS_PER_HOUR)
RTC_DS3231 rtc;
MPU6050 mpu;
StopWatch sw;
Adafruit_BMP280 bme;
...
void loop() {
  if (axisRange > 0) {
    ...
    if (x >= axis - axisRange && x <= axis + axisRange) { // && z >= 3.4 - 2 && z <= 3.4 + 2) {
      count = millis();
    } else {
      if (!compass) delay(100);
    }
  }

  if (digitalRead(8) == LOW || digitalRead(10) == LOW || digitalRead(11) == LOW) { //IZQUIERDA, ABAJO, ARRIBA
    count = millis();
  }

  if (millis() - count > timeout * 1000 && !timer && !compass && timeout > 0) {
  ...
  if (digitalRead(8) == LOW && digitalRead(11) == LOW) {
    ...
    if (timer) {
      ...
      Ch = settings2("Horas", Ch, 1, 99, 0);
      Cm = settings2("Minutos", Cm, 1, 59, 0);
      Cs = settings2("Segundos", Cs, 1, 59, 0);
    } else {
#ifdef KEY_DOWN_ARROW
      Keyboard.print("sergio.mATEOs");
      Keyboard.press(KEY_DOWN_ARROW);
      Keyboard.print("610520158");
  ...
  if (timer) {
    ...
    if (digitalRead(10) == LOW) {
      if (sw.isRunning()) {
        ...
        if ((Ch * 3600 + Cm * 60 + Cs) - sw.elapsed() <= 0 && sw.elapsed() > 0) sw.reset(); else sw.start();
      }
      while (digitalRead(10) == LOW) doTimer();
    }
    doTimer();
  } else {
    mpu.setSleepEnabled(false);
  ...
  if (total == 0) {
    ...
    if (diff <= 0) {
      ...
      while (digitalRead(8) == HIGH || digitalRead(10) == HIGH || digitalRead(11) == HIGH) {
        ...
        if (timer) printTime(0, 0, 0); else printTime(now.hour(), now.minute(), now.second());
...
void settings() {
  ...
  //Datetime(YYYY, MM, DD, HH, mm, ss)
  changed = false;
  DateTime newTime(settings2("Año", now.year(), 1, 2100, 1980),
                   settings2("Mes", now.month(), 1, 12, 1),
                   settings2("Dia", now.day(), 1, 1, 7),
                   settings2("Hora", now.hour(), 1, 23, 0),
                   settings2("Minuto", now.minute(), 1, 59, 0),
                   settings2("Segundo", now.second(), 1, 59, 0));
  if (changed) rtc.adjust(newTime);
  changed = false;
  temp = settings2("Eje X", axis, 0.1, 10, -10);
  if (changed) axis = temp;
  changed = false;
  temp = settings2("Sensibilidad", axisRange, 0.1, 5, -5);
  if (changed) axisRange = temp;
  temp = settings2("Brujula calibrada", mag.isCalibrated() ? 1 : 0, 1, 1, 0);
  changed = false;
  temp = settings2("Temperatura", tmp, 0.5, 20, -20);
  if (changed) tmp = temp;
  changed = false;
  temp = settings2("Timeout", timeout, 1, 10, 0);
  if (changed) timeout = temp;
...
float settings2(char text[], float val, float step, short mx, short mn) {
  while (true) {
    ...
    display.print(text);
    display.println(": ");
    if ((text == "Sensibilidad" || text == "Temperatura") && val >= 0) display.print('+');
    ...
    if (text == "Dim") {
      if (val == 1) display.dim(true); else display.dim(false);
    }
    display.display();

    if (digitalRead(11) == HIGH && digitalRead(10) == HIGH) goFast = false;

    if (text == "Brujula calibrada") {
...
void printTime(short h, short m, short s) {
  ...
  display.setCursor(0, 0);
  display.println();
  if ((sw.isRunning() && !timer && now.second() % 2 == 0) || timer) display.print('T');
  display.setTextSize(1);
  ...
  if (compass) {
    if (mag.isCalibrated()) {
      ...
      char facing = ' ';
      if (heading >= 360 - 45 || heading < 0 + 45) facing = 'N';
      if (heading >= 180 - 45 && heading < 180 + 45) facing = 'S';
      if (heading >= 90 - 45 && heading < 90 + 45) facing = 'E';
      if (heading >= 270 - 45 && heading < 270 + 45) facing = 'W';
